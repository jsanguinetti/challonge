...
import { ICreateTournament, ITournament, IUpdateTournament } from './tournament.interface';
...
export class TournamentsService {
  ...
  public async update(tournamentParams: IUpdateTournament): Promise<ITournament> {
    const tournament = await this.find(tournamentParams.id);
    const tournamentToUpdate = await this.mergeTournaments(tournament, tournamentParams.challongeIdAlias);
    const updatedTournament = await this.tournamentRepository.save(tournamentToUpdate);
    return updatedTournament.toJSON();
  }

  public async delete(tournamentId): Promise<boolean> {
    const deleteResult = await this.tournamentRepository.delete({ id: tournamentId });
  ...
  private async createTournament(tournamentParams: ICreateTournament) {
    return this.mergeTournaments(this.tournamentRepository.create(), tournamentParams.challongeIdAlias);
  }

  private async mergeTournaments(tournament: Tournament, challongeTournamentIdAlias: string) {
    const challongeTournament = await this.challongeService.getTournament(challongeTournamentIdAlias);
