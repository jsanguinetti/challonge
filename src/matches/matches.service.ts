...
type ChallongeMapperFunction = (challongeMatch: IChallongeMatch) => Promise<IMatch>;
...
export class MatchesService {
  ...
  public async list(listMatchesParams: ListMatchesParams): Promise<IMatch[]> {
    ...
    const challongeMatches = await this.challongeService.getMatches(tournament.challongeId, user.challongeId);
    const matches = await Promise.all(challongeMatches.map(this.buildMatch(user)));
    return matches;
  }
  ...
  private buildMatch(user: IUser): ChallongeMapperFunction {
    ...
    const findUser = async function (id: number): Promise<IUser> {
      ...
      if (foundUser) {
        ...
        const challongeUser = await this.userService.findOrCreateFromChallongeId(id);
    ...
    const mapperFunction = async function (challongeMatch: IChallongeMatch) {
      ...
      if (challongeMatch.player1Id == user.challongeId)
        player1 = user;
      else
        player1 = await findUser(challongeMatch.player1Id);

      if (challongeMatch.player2Id == user.challongeId)
        player2 = user;
      else
        player2 = await findUser(challongeMatch.player2Id);

      adversary = player1.id == user.id ? player1 : player2;
